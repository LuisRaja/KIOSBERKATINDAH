...
function renderProductGrid() {
    ...
    productGrid.innerHTML = filtered.map(prod => {
        ...
        return `
            <div class="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between transition-all duration-300 hover:shadow-md group ${outOfStock ? 'opacity-60' : ''}">
                <div>
                    ${hasImage ? `<div class="w-full h-32 bg-gray-100 rounded-xl mb-3 overflow-hidden flex items-center justify-center">
                        <img src="${prod.image}" alt="${prod.nama}" class="w-full h-full object-cover" onerror="this.style.display='none'" loading="lazy">
                    </div>` : ''}
                    <div class="flex items-center justify-between mb-3">
                        <span class="text-[9px] uppercase font-extrabold tracking-widest text-gold bg-amber-50 px-2.5 py-1 rounded-md">${prod.kategori}</span>
                        <span class="text-[11px] text-gray-400 font-medium">Stok: <span class="${stock <= 5 ? 'text-red-500 font-bold' : 'text-burgundy font-bold'}">${stock}</span></span>
                    </div>
                    <h3 class="font-bold text-gray-800 tracking-tight text-base group-hover:text-burgundy transition-colors line-clamp-2">${prod.nama}</h3>
                    ${outOfStock ? '<p class="text-[10px] text-red-500 font-bold mt-1">STOK HABIS</p>' : ''}
                </div>
                <div class="mt-5">
                    <p class="text-lg font-black text-burgundy mb-4">${formatRupiah(prod.harga)}</p>
                    ${outOfStock ? `<button disabled class="w-full bg-gray-200 text-gray-400 font-bold py-2.5 px-4 rounded-xl text-xs cursor-not-allowed">Stok Habis</button>`
                    : cartQty > 0
                        ? `<div class="flex items-center justify-between bg-burgundy rounded-xl p-1 text-white shadow-inner">
                            <button onclick="updateCartQty('${prod.id}', -1)" class="p-2 hover:bg-white hover:bg-opacity-10 rounded-lg" aria-label="Kurangi"><i data-lucide="minus" class="w-4 h-4 text-gold"></i></button>
                            <span class="font-bold text-sm">${cartQty}</span>
                            <button onclick="updateCartQty('${prod.id}', 1)" class="p-2 hover:bg-white hover:bg-opacity-10 rounded-lg" ${cartQty >= MAX_LIMIT_PER_PRODUCT ? 'disabled aria-disabled="true"' : ''} aria-label="Tambah"><i data-lucide="plus" class="w-4 h-4 text-gold"></i></button>
                        </div>`
                        : `<button onclick="updateCartQty('${prod.id}', 1)" class="w-full border border-burgundy border-opacity-40 text-burgundy hover:bg-burgundy hover:text-white font-bold py-2.5 px-4 rounded-xl transition-all duration-300 text-xs tracking-wider uppercase flex items-center justify-center gap-2" aria-label="Tambah ${prod.nama}">
                            <i data-lucide="plus" class="w-3.5 h-3.5"></i> Tambah Belanja
                        </button>`
                    }
                </div>
            </div>`;
...
function updateHeaderCartBadge() {
    ...
    if (totalItems > 0) {
        ...
        cartBadge.classList.replace('scale-0', 'scale-100');
    } else {
        cartBadge.classList.replace('scale-100', 'scale-0');
    }
}
    ...
    if (cartItemIds.length === 0) {
        cartItemsContainer.innerHTML = `
            <div class="h-full flex flex-col items-center justify-center text-center text-gray-400 py-16">
                <i data-lucide="shopping-bag" class="w-12 h-12 text-gray-200 mb-2"></i>
...
function openAuthModal() {
    ...
    document.getElementById('auth-modal-sub').textContent = 'Buat akun untuk melanjutkan pesanan';
...
function showAuthForm(type) {
    ...
    if (type === 'phone') {
        ...
        document.getElementById('auth-phone-group').classList.remove('hidden');
        ...
        document.getElementById('auth-phone-group').classList.add('hidden');
        document.getElementById('auth-name-group').classList.remove('hidden');
    }
    document.getElementById('auth-name-group').classList.remove('hidden');
    document.getElementById('auth-password-group').classList.remove('hidden');
    document.getElementById('auth-name').value = '';
    document.getElementById('auth-email').value = '';
    document.getElementById('auth-password').value = '';
    if (document.getElementById('auth-phone-input')) document.getElementById('auth-phone-input').value = '';
    document.getElementById('btn-auth-submit').textContent = 'Daftar';
    document.getElementById('auth-toggle-text').textContent = 'Sudah punya akun?';
    document.getElementById('auth-toggle-btn').textContent = 'Masuk';
    lucide.createIcons();
}
...
function toggleAuthMode() {
    ...
    document.getElementById('auth-modal-title').textContent = isLoginMode ? 'Masuk' : 'Daftar Akun';
    document.getElementById('auth-modal-sub').textContent = isLoginMode ? 'Masuk dengan akun Anda' : 'Buat akun untuk melanjutkan pesanan';
    document.getElementById('btn-auth-submit').textContent = isLoginMode ? 'Masuk' : 'Daftar';
    document.getElementById('auth-toggle-text').textContent = isLoginMode ? 'Belum punya akun?' : 'Sudah punya akun?';
    document.getElementById('auth-toggle-btn').textContent = isLoginMode ? 'Daftar' : 'Masuk';
...
async function submitAuth() {
    ...
    const phone = document.getElementById('auth-phone-input') ? document.getElementById('auth-phone-input').value.trim() : '';
    ...
    if (authMode === 'email') {
        ...
        if (!phone || !password) {
            errorEl.textContent = 'Nomor telepon dan password wajib diisi';
            errorEl.classList.remove('hidden');
            return;
        }
        if (!isLoginMode && !validatePhone(phone)) {
            errorEl.textContent = 'Nomor telepon tidak valid (harus 08xx, min. 10 digit)';
            errorEl.classList.remove('hidden');
            return;
        }
    }
    ...
    try {
        ...
        const body = isLoginMode
            ? (authMode === 'email' ? { email, password } : { phone, password })
            : (authMode === 'email' ? { email, password, name, phone: phone || '' } : { phone, password, name, email: email || '' });
        ...
        if (res.success) {
            ...
            errorEl.textContent = res.error || 'Gagal';
            errorEl.classList.remove('hidden');
        }
    } catch (e) {
